...
public class MqttConsumerService : IHostedService, IDisposable
{
    ...
    public MqttConsumerService(ILogger<MqttConsumerService> logger)
    {
        ...
        _brokerPort = int.TryParse(Environment.GetEnvironmentVariable("MQTT_BROKER_PORT"), out var port) ? port : 1883;
        
        var instanceId = Environment.GetEnvironmentVariable("MQTT_CONSUMER_INSTANCE") ?? Guid.NewGuid().ToString();
    ...
    private async Task ReconnectAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                _logger.LogInformation("Attempting to reconnect to MQTT Broker (Attempt {Attempt})...", attempt);
