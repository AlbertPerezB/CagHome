...
```json
{
    "Simulator": {
        "Profile": "normal",
        "DeviceCount": 3,
        "PublishBiometricsIntervalSeconds": 10,
        "PublishBatchIntervalSeconds": 30
    }
}
```
...
The status field is an integer corresponding to the patient status enum values: <br>
0. Active
1. Inactive
2. Deceased

### Configuring simulated patients 
_(new functionality added to support load testing see below)_

The mock application and mock EHR are independent services and do not share patient information automatically.

After seeding a patient in the mock EHR, the same patient ID must be added to the Mock Application configuration so that the simulator can generate telemetry for that patient.

Update the `PatientIds` setting in:
...
```json
{
  "Simulator": {
    "Profile": "normal",
    "PublishBatchIntervalSeconds": 10,
    "SampleBiometricsIntervalSeconds": 3,
    "PatientIds": [
      "12345678-47ef-0002-9a7a-123456789123",
      "87654321-47ef-0002-9a7a-987654321987"
    ]
  }
}
```

Each patient ID listed in `PatientIds` should correspond to a patient that has been seeded in the mock EHR.

If no matching patient exists in the mock EHR, telemetry will still be generated by the simulator, but downstream services will not be able to associate the measurements with a registered patient.

#### Example patients

For convenience, the following three patients can be seeded and used for demonstration purposes:

**Patient 1 – Coronary artery disease**
...
**Patient 2 – Valve disease**

```json
{
  "PatientId": "8f3d2a11-6b0a-4d91-8f5c-3b0f0b7c9e41",
  "UpdatedAtUtc": "2026-05-28T16:09:00Z",
  "Careplan": 1,
  "Status": 0
}
...
**Patient 3 – Cardiomyopathy**

```json
{
  "PatientId": "c72a1e8d-9f6e-42b8-b8e9-7f4f0f3d2c55",
  "UpdatedAtUtc": "2026-05-28T16:09:00Z",
  "Careplan": 3,
  "Status": 0
}
...
To simulate telemetry for all three patients, add them to the appsettings in mock application:
...
```json
{
  "Simulator": {
    ...
    "PatientIds": [
      "12345678-47ef-0002-9a7a-123456789123",
      "8f3d2a11-6b0a-4d91-8f5c-3b0f0b7c9e41",
      "c72a1e8d-9f6e-42b8-b8e9-7f4f0f3d2c55"
    ]
  }
}
```

#### Registering patients automatically
In order to support load testing through the mock application, a new setting has been added, which can toggle whether patients should automatically 
be registered. If the registerpatients flag in the appsettings is set to true, the program will use device count and generate that amount of new patient ids and register them.
In this case, the patient ids in the appsettings will be ignored. If the register patients flag is false, the devicecount setting will be ignored.

All patients registered will have the same profile, and will all have valvedisease as careplan.
